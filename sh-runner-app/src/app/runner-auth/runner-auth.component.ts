...
export class RunnerAuthComponent implements OnInit {
  ...
  authenticate(){
    let server_auth = this.setupService.post('http://'+ this.form.controls["host"].value +':'+ this.form.controls["port"].value +'/server_signin',{ "username":this.form.controls["username"].value, "password":this.form.controls["password"].value });
